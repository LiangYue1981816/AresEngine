...
bool CVKIndirectBuffer::BufferData(uint32_t count, const DrawCommand* data)
{
	...
	if (m_pBuffer->GetBufferSize() < sizeof(DrawCommand) * count) {
		...
		m_pBuffer = new CVKBuffer(m_pDevice, newSize, VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, CGfxProfiler::BufferType::BUFFER_TYPE_INDIRECT_BUFFER);
	}
