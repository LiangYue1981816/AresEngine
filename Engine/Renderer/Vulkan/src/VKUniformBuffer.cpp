...
CVKUniformBuffer::CVKUniformBuffer(CVKDevice *pDevice, size_t size)
	...
	, m_pDevice(pDevice)
{
	m_ptrBuffer = CVKBufferPtr(new CVKBuffer(m_pDevice, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, CGfxSwapChain::SWAPCHAIN_FRAME_COUNT * m_size));
}
