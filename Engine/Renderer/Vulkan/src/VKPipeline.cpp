...
CVKPipeline::CVKPipeline(CVKDevice* pDevice, VkPipelineCache vkPipelineCache, const CGfxShader* pComputeShader)
	: m_pDevice(pDevice)
	, m_pShaders{ nullptr }
	...
	, m_vkPipelineCache(vkPipelineCache)
{
	...
	for (int indexAttachment = 0; indexAttachment < std::min((int)pRenderPass->GetSubpassOutputAttachmentCount(indexSubpass), 4); indexAttachment++) {
		...
		colorBlendAttachment.dstColorBlendFactor = CVKHelper::TranslateBlendFactor(state.blendDstRGB[indexAttachment]);
		colorBlendAttachment.colorBlendOp = CVKHelper::TranslateBlendOp(state.blendOpRGB[indexAttachment]);
		colorBlendAttachment.srcAlphaBlendFactor = CVKHelper::TranslateBlendFactor(state.blendSrcAlpha[indexAttachment]);
		colorBlendAttachment.dstAlphaBlendFactor = CVKHelper::TranslateBlendFactor(state.blendDstAlpha[indexAttachment]);
		colorBlendAttachment.alphaBlendOp = CVKHelper::TranslateBlendOp(state.blendOpAlpha[indexAttachment]);
		colorBlendAttachment.colorWriteMask = (state.bEnableColorRedWrite[indexAttachment] ? VK_COLOR_COMPONENT_R_BIT : 0) | (state.bEnableColorGreenWrite[indexAttachment] ? VK_COLOR_COMPONENT_G_BIT : 0) | (state.bEnableColorBlueWrite[indexAttachment] ? VK_COLOR_COMPONENT_B_BIT : 0) | (state.bEnableColorAlphaWrite[indexAttachment] ? VK_COLOR_COMPONENT_A_BIT : 0);
...
{
	m_ptrDescriptorLayouts[DESCRIPTOR_SET_PASS] = new CVKDescriptorLayout(m_pDevice, DESCRIPTOR_SET_PASS);
	m_ptrDescriptorLayouts[DESCRIPTOR_SET_MATPASS] = new CVKDescriptorLayout(m_pDevice, DESCRIPTOR_SET_MATPASS);
	m_ptrDescriptorLayouts[DESCRIPTOR_SET_MESHDRAW] = new CVKDescriptorLayout(m_pDevice, DESCRIPTOR_SET_MESHDRAW);
	m_ptrDescriptorLayouts[DESCRIPTOR_SET_INPUTATTACHMENT] = new CVKDescriptorLayout(m_pDevice, DESCRIPTOR_SET_INPUTATTACHMENT);
