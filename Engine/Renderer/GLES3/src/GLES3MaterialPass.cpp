...
bool CGLES3MaterialPass::SetSampler(const char *szName, int mipLevels, GfxFilter minFilter, GfxFilter magFilter, GfxSamplerMipmapMode mipmapMode, GfxSamplerAddressMode addressMode)
{
	uint32_t name = HashValue(szName);

	if ((m_pPipeline == nullptr) || (m_pPipeline && m_pPipeline->IsTextureValid(name))) {
		m_pSamplers[name] = GLES3Renderer()->CreateSampler(mipLevels, minFilter, magFilter, mipmapMode, addressMode);
		return true;
	}
...
bool CGLES3MaterialPass::SetTexture2D(const char *szName, uint64_t externTexture)
{
	uint32_t name = HashValue(szName);

	if ((m_pPipeline == nullptr) || (m_pPipeline && m_pPipeline->IsTextureValid(name))) {
		m_ptrTexture2Ds[name] = GLES3Renderer()->NewTexture2D(TEXTURE_INTERNAL_NAME(name));
		m_ptrTexture2Ds[name]->Create(externTexture);
		return true;
	}
...
bool CGLES3MaterialPass::SetTexture2DArray(const char *szName, uint64_t externTexture)
{
	uint32_t name = HashValue(szName);

	if ((m_pPipeline == nullptr) || (m_pPipeline && m_pPipeline->IsTextureValid(name))) {
		m_ptrTexture2DArrays[name] = GLES3Renderer()->NewTexture2DArray(TEXTURE_INTERNAL_NAME(name));
		m_ptrTexture2DArrays[name]->Create(externTexture);
		return true;
	}
...
bool CGLES3MaterialPass::SetTextureCubeMap(const char *szName, uint64_t externTexture)
{
	uint32_t name = HashValue(szName);

	if ((m_pPipeline == nullptr) || (m_pPipeline && m_pPipeline->IsTextureValid(name))) {
		m_ptrTextureCubeMaps[name] = GLES3Renderer()->NewTextureCubeMap(TEXTURE_INTERNAL_NAME(name));
		m_ptrTextureCubeMaps[name]->Create(externTexture);
		return true;
	}
