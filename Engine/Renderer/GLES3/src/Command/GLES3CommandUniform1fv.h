...
{
public:
	CGLES3CommandUniform1fv(const CGfxPipelineCompute* pPipelineCompute, const CGfxPipelineGraphics* pPipelineGraphics, uint32_t name, int count, const float* value)
	...
	virtual void Execute(void) const
	{
		...
		CGfxProfilerSample sample(CGfxProfiler::SAMPLE_TYPE_COMMAND_UNIFORM1FV);
		{
			if (m_pPipelineCompute) {
				m_pPipelineCompute->Uniform1fv(m_name, m_value.size(), m_value.data());
			}

			if (m_pPipelineGraphics) {
				m_pPipelineGraphics->Uniform1fv(m_name, m_value.size(), m_value.data());
			}
		}
	}
