...
{
	return m_pShaders[shaderc_fragment_shader] ? m_pShaders[shaderc_fragment_shader]->IsTextureValid(name) : false;
}

bool CGfxPipelineBase::BindTexture2D(uint32_t name, uint32_t texture, uint32_t sampler, uint32_t unit) const
{
	return m_pShaders[shaderc_fragment_shader] ? m_pShaders[shaderc_fragment_shader]->BindTexture2D(name, texture, sampler, unit) : false;
}

bool CGfxPipelineBase::BindTexture2DArray(uint32_t name, uint32_t texture, uint32_t sampler, uint32_t unit) const
{
	return m_pShaders[shaderc_fragment_shader] ? m_pShaders[shaderc_fragment_shader]->BindTexture2DArray(name, texture, sampler, unit) : false;
}

bool CGfxPipelineBase::BindTextureCubeMap(uint32_t name, uint32_t texture, uint32_t sampler, uint32_t unit) const
{
	return m_pShaders[shaderc_fragment_shader] ? m_pShaders[shaderc_fragment_shader]->BindTextureCubeMap(name, texture, sampler, unit) : false;
}
...
bool CGfxPipelineBase::BindUniformBuffer(uint32_t name, uint32_t buffer, uint32_t size, int offset) const
{
	for (int index = 0; index < shaderc_compute_shader - shaderc_vertex_shader + 1; index++) {
		if (m_pShaders[index]) {
			m_pShaders[index]->BindUniformBuffer(name, buffer, size, offset);
		}
	}
	return true;
}
