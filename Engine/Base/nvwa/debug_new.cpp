...
static void print_position(const void* ptr, int line)
{
    ...
    {
        LogOutput("Memory", "%s:%d", (const char*)ptr, line);
    }
    else if (ptr != NULL)   // Is caller address present?
    {
		LogOutput("Memory", "%p", ptr);
    }
    else                    // No information is present
    {
		LogOutput("Memory", "<Unknown>");
    }
}
...
void free_pointer(void* usr_ptr, void* addr, bool is_array)
{
    ...
    {
        {
			LogOutput("Memory", "delete%s: invalid pointer %p (", is_array ? "[]" : "", usr_ptr);
...
int dump_memory_objects()
{
	...
	for (new_ptr_list_t* ptr = new_ptr_list.next; ptr != &new_ptr_list; ptr = ptr->next)
    {
        ...
        if (ptr->magic != DEBUG_NEW_MAGIC)
        {
			LogOutput("Memory", "warning: heap data corrupt near %p\n", usr_ptr);
        }
#if _DEBUG_NEW_TAILCHECK
        if (!check_tail(ptr))
        {
			LogOutput("Memory", "warning: overwritten past end of object at %p\n", usr_ptr);
        }
#endif
		LogOutput("Memory", "object at %p (size %lu, ", usr_ptr, (unsigned long)ptr->size);
        if (ptr->line != 0)
            print_position(ptr->file, ptr->line);
        else
            print_position(ptr->addr, ptr->line);
...
int check_mem_corruption()
{
    int corrupt_cnt = 0;
    fast_mutex_autolock lock_ptr(new_ptr_lock);
	LogOutput("Memory", "*** Checking for memory corruption: START\n");
    for (new_ptr_list_t* ptr = new_ptr_list.next; ptr != &new_ptr_list; ptr = ptr->next)
    {
        const char* const usr_ptr = (char*)ptr + ALIGNED_LIST_ITEM_SIZE;
        if (ptr->magic == DEBUG_NEW_MAGIC
        ...
        {
#endif
			LogOutput("Memory", "Heap data corrupt near %p (size %lu, ", usr_ptr, (unsigned long)ptr->size);
#if _DEBUG_NEW_TAILCHECK
        }
        else
        {
			LogOutput("Memory", "Overwritten past end of object at %p (size %lu, ", usr_ptr, (unsigned long)ptr->size);
        }
#endif
        if (ptr->line != 0)
            print_position(ptr->file, ptr->line);
	...
	LogOutput("Memory", "*** Checking for memory corruption: %d FOUND\n", corrupt_cnt);
    return corrupt_cnt;
}
