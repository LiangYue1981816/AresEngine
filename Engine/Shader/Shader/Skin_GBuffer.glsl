...
#endif
DESCRIPTOR_SET_MATPASS(12) uniform sampler2D texSkinLUT;
...
void main()
{
	...
	vec4 projectCoord = cameraProjectionViewMatrix * vec4(worldPosition, 1.0);
	...
	float shadow = ShadowValue(worldPosition, inNormal, texShadow);
//	float shadow = ShadowValueIrregular(worldPosition, texShadow);

	vec3 pointLightDirection = mainPointLightPosition - worldPosition;
	...
	vec3 ambientLighting = AmbientSH9(worldNormal, albedoColor, 0.0) * ambientLightFactor;
	vec3 pointLighting = SkinLighting(inNormal, worldNormal, worldPosition, worldViewDirection, pointLightDirection, pointLightColor, albedoColor, roughness, texSkinLUT) * pointLightFactor;
	vec3 directLighting = SkinLighting(inNormal, worldNormal, worldPosition, worldViewDirection, mainDirectLightDirection, mainDirectLightColor, albedoColor, roughness, texSkinLUT) * directLightFactor;
#ifdef ENV_MAP
