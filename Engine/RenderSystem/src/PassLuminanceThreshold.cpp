...
void CPassLuminanceThreshold::Render(CTaskPool& taskPool, CTaskGraph& taskGraph, CGfxCommandBufferPtr ptrCommandBuffer)
{
	// Update
	...
	GfxRenderer()->CmdPushDebugGroup(ptrCommandBuffer, "PassLuminanceThreshold");
	{
		...
		GfxRenderer()->CmdBeginRenderPass(ptrCommandBuffer, m_ptrFrameBuffer, ptrRenderPass);
		{
			...
			m_pRenderQueue->CmdDraw(taskPool, taskGraph, ptrCommandBuffer, m_ptrDescriptorSetPass, PASS_LUMINANCE_THRESHOLD_NAME, scissor, viewport, znear, zfar, 0xffffffff, false);
		}
