...
void CPassBlend::Create(GfxPixelFormat colorPixelFormat)
{
...
void CPassBlend::Destroy(void)
{
	ptrRenderPass.Release();
}


CPassBlend::CPassBlend(CRenderSystem* pRenderSystem)
	: CPassBlit(PASS_BLEND_MATERIAL_NAME, pRenderSystem)
{
	// CommandBuffer
...
void CPassBlend::SetCamera(CCamera* pCamera)
{
...
const CGfxSemaphore* CPassBlend::Render(CTaskGraph& taskGraph, const CGfxSemaphore* pWaitSemaphore)
{
	// Update
	...
	const CGfxCommandBufferPtr ptrMainCommandBuffer = m_ptrMainCommandBuffer[GfxRenderer()->GetSwapChain()->GetFrameIndex()];
	{
		...
		GfxRenderer()->BeginRecord(ptrMainCommandBuffer);
		{
			...
			GfxRenderer()->CmdBeginRenderPass(ptrMainCommandBuffer, m_ptrFrameBuffer, ptrRenderPass);
			{
				...
				m_pRenderQueue->CmdDraw(taskGraph, ptrMainCommandBuffer, m_ptrDescriptorSetPass, PASS_BLEND_NAME, scissor, viewport, 0xffffffff);
			}
