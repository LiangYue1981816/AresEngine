	...
	: CPassBlit(PASS_COPY_MATERIAL_NAME, pRenderSystem)
{
	...
	m_ptrDescriptorSetPass = GfxRenderer()->NewDescriptorSet(PASS_COPY_NAME, ptrDescriptorLayout);
...
void CPassCopy::Render(CTaskGraph& taskGraph, CGfxCommandBufferPtr ptrMainCommandBuffer)
{
	...
	GfxRenderer()->CmdBeginRenderPass(ptrMainCommandBuffer, m_ptrFrameBuffer, ptrRenderPass);
	{
		...
		m_pRenderQueue->CmdDraw(&taskGraph, ptrMainCommandBuffer, m_ptrDescriptorSetPass, PASS_COPY_NAME, scissor, viewport, 0xffffffff);
	}
