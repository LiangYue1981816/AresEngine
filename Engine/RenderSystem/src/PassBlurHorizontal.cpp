	...
	, m_range(1.0f)
{
	...
	m_ptrDescriptorSetPass = GfxRenderer()->NewDescriptorSet(PASS_BLUR_HORIZONTAL_NAME, ptrDescriptorLayout);
...
void CPassBlurHorizontal::Render(CTaskGraph& taskGraph, CGfxCommandBufferPtr ptrMainCommandBuffer)
{
	// Update
	...
	GfxRenderer()->CmdBeginRenderPass(ptrMainCommandBuffer, m_ptrFrameBuffer, ptrRenderPass);
	{
		...
		m_pRenderQueue->CmdDraw(&taskGraph, ptrMainCommandBuffer, m_ptrDescriptorSetPass, PASS_BLUR_HORIZONTAL_NAME, scissor, viewport, 0xffffffff);
	}
