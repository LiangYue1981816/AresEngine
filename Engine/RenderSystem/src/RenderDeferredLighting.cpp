...
void CRenderSystem::RenderDeferredLighting(CTaskGraph& taskGraph, CCamera* pCamera, bool bPresent)
{
	...
	GfxRenderer()->AcquireNextFrame();
	{
		...
		{
			RenderDeferredLighting(taskGraph, pCamera, bPresent, ptrGraphicCommandBuffer);
		}
...
void CRenderSystem::RenderDeferredLighting(CTaskGraph& taskGraph, CCamera* pCamera, bool bPresent, CGfxCommandBufferPtr ptrCommandBuffer)
{

}

void CRenderSystem::RenderTileBaseDeferredLighting(CTaskGraph& taskGraph, CCamera* pCamera, bool bPresent)
{
	m_pInstanceBufferPool->Clear();
...
void CRenderSystem::RenderTileBaseDeferredLighting(CTaskGraph& taskGraph, CCamera* pCamera, bool bPresent, CGfxCommandBufferPtr ptrCommandBuffer)
{
	uint32_t rtFinal;
	...
	uint32_t rtGBuffer2 = RENDER_TEXTURE_GBUFFER2;
	{
		...
		m_pPassDeferredLighting->Render(taskGraph, ptrCommandBuffer);
	}
