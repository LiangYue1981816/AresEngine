...
CPassBlurVertical::CPassBlurVertical(CRenderSystem* pRenderSystem)
	: CPassBlit("PassBlurVertical.material", pRenderSystem)
	, m_range(1.0f)
{
...
void CPassBlurVertical::Render(CTaskPool& taskPool, CTaskGraph& taskGraph, CGfxCommandBufferPtr ptrCommandBuffer)
{
	...
	GfxRenderer()->CmdPushDebugGroup(ptrCommandBuffer, "PassBlurVertical");
	{
		...
		GfxRenderer()->CmdBeginRenderPass(ptrCommandBuffer, m_ptrFrameBuffer, ptrRenderPass);
		{
			...
			m_pRenderQueue->CmdDraw(taskPool, taskGraph, ptrCommandBuffer, m_ptrDescriptorSetPass, PASS_BLUR_VERTICAL_NAME, scissor, viewport, znear, zfar, 0xffffffff, false);
		}
