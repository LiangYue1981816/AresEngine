	...
	: CPassBlit(PASS_COLOR_GRADING_MATERIAL_NAME, pRenderSystem)
{
	...
	m_ptrDescriptorSetPass = GfxRenderer()->NewDescriptorSet(HashValueFormat("%x_%p", PASS_COLOR_GRADING_NAME, this), ptrDescriptorLayout);
...
void CPassColorGrading::Render(CTaskPool& taskPool, CTaskGraph& taskGraph, CGfxCommandBufferPtr ptrCommandBuffer)
{
	// Update
	...
	GfxRenderer()->CmdPushDebugGroup(ptrCommandBuffer, "PassColorGrading");
	{
		...
		GfxRenderer()->CmdBeginRenderPass(ptrCommandBuffer, m_ptrFrameBuffer, ptrRenderPass);
		{
			...
			m_pRenderQueue->CmdDraw(taskPool, taskGraph, ptrCommandBuffer, m_ptrDescriptorSetPass, PASS_COLOR_GRADING_NAME, scissor, viewport, 0xffffffff, false);
		}
